...
- Keep view-mode values 0-16 synchronized across HTML radios, `VIEW_NAMES`, `this.legends`, render-shader branches, overlays, and download names. View 9 is the current annualized precipitation rate; view 10 sums the twelve monthly precipitation channels; views 11-13 read the twelve monthly surface-air temperature channels for annual mean, coldest monthly mean, and warmest monthly mean. View 16 averages the separate monthly SST textures and masks the temperature palette to ocean. View 14 reads the separate monthly SST channels and masks them to ocean for year-round sea ice; view 15 reads surface-air temperature and masks it to land for permafrost. Both classifications require a warmest monthly mean below 0 °C.
...
- Current, annual-mean surface-air, coldest-month, warmest-month, and annual-mean SST temperature views share one −80 °C to 50 °C normalization. Keep the GLSL palette and CSS legends synchronized: 0 °C is green, subzero values progress through cyan/blue/violet, and positive values progress through yellow/orange/red.
- Wind climatology uses three RG16F textures: two ping-pong textures accumulate the current year's vector mean, while the third retains the completed year used by atmospheric temperature. At year rollover, never clear or attach the retained texture for writing until a newer completed mean replaces it. Treat the first automatic year as wind spin-up: discard its transitional monthly patterns and do not classify it; classify the following full year.
- Wind-driven maritime temperature traces six samples upwind through SST. Reach grows continuously with vector-mean wind speed, longitude distance is latitude-corrected, distant sources decay, influence is capped, the ocean mask fades only on the water side, and elevation cooling remains intact. Preserve direction reversal under retrograde circulation and cancellation of seasonally reversing winds.
...
- Surface-ocean western-boundary intensification follows the sign of rotation. Its mid-latitude separated jet and current-aligned upstream heat trace must remain longitude-wrapped, blocked by intervening land, speed-capped, and timestep-scaled; fast-current thermal memory must fade toward polar oceans.
